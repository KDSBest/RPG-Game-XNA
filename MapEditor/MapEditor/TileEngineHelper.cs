            ...
            for (int y = 0; y < map.MapDimensions.Y; y++)
            {
                for (int x = 0; x < map.MapDimensions.X; x++)
                {
                    ...
                    if (CheckVisibility(destinationRectangle))
                    {
                        ...
                        if (this.PartyLeaderPosition.TilePosition.X == x && this.PartyLeaderPosition.TilePosition.Y == y)
                        {
                            Globals.Instance.SpriteBatch.Draw(Globals.Instance.PixelWhite, destinationRectangle, Color.White * 0.2f);
                        }
                    }
                }
            }
